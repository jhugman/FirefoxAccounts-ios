...
open class FxALoginHelper {
    public static var sharedInstance: FxALoginHelper = {
    ...
    public func application(_ application: UIApplication, didLoadProfile profile: Profile) {
    ...
    fileprivate func awaitVerification(_ attemptsLeft: Int = verificationMaxRetries) {
        ...
        account.advance().upon { state in
            guard state.actionNeeded == .needsVerification else {
                ...
                // we're done.
//                self.delegate?.accountDidVerify()
